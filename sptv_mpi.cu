...
void computePI(int nproc,int rank);
{
	...
	for (i = 0; i < nnz; i++)
	{
		...
		if( ((column-1)>=(rank*num_column/nproc)) && ((column-1)<((rank+1)*num_column/nproc)))
		{
			rows[g] = row - 1;
			columns[g] = column - (rank*num_column/nproc) - 1;
			g++;
		}
	}
	nnz=g;
	num_column = ((rank+1)*num_column/nproc) - (rank*num_column/nproc);
